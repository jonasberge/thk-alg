package de.thkoeln.inf.agelb.visualisierung.adt.queue
...
class IndexedPriorityQueue<T>(size: Int) : Collection<T> {
    ...
    fun insert(i: Int, key: T) {
        if (i < 0 || i >= maxSize) throw IndexOutOfBoundsException()
    ...
    fun decreaseKey(i: Int, key: T) {
        ...
        require(greater(keys[i]!!, key)) { "Calling decreaseKey() with given argument would not strictly decrease the key" }
        keys[i] = key
        swim(qp[i])
    }
    ...
    fun increaseKey(i: Int, key: T) {
        if (i < 0 || i >= maxSize) throw IndexOutOfBoundsException()
        if (!contains(i)) throw NoSuchElementException("index is not in the priority queue")
        require(less(keys[i]!!, key)) { "Calling increaseKey() with given argument would not strictly increase the key" }
        keys[i] = key
        sink(qp[i])
    }
    ...
    val indices get() = pq.copyOfRange(1, size + 1).toSet()
}